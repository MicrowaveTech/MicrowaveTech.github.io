...
模拟交易正常采集上传。临时测试回测时，配置 `allow_backtest: true`，且 transport 保持 github；支持编译运行和完整回测，未知运行模式仍不会上传。包内已加入该开关，但总开关 enabled 默认仍关闭。
测试完成后将 allow_backtest 改为 false，即恢复仅模拟交易同步。普通聚宽日志不受这个开关影响。
...
回测记录真实标注 `mode: backtest`，每次初始化产生独立运行编号，上传到：

```text
quant/_quant/inbox/four-strategies/backtests/运行编号/交易日期/批次编号.json
```

看板顶部“数据来源”可选择模拟交易或某次回测；选中回测后显示“回测数据”提示。相同日期的模拟交易和不同回测不会合并。
这次需同时更新完整策略代码和网站 quant 文件夹中的解析器、网页源文件，原 workflow 不用改；只改 JSON 开关无法让旧版解析器识别回测。
回测任务按历史交易时钟触发上传，不是现实时间每小时一次。建议先跑 1～3 个交易日验证，长区间回测会产生较多网络请求和提交，影响运行耗时。
...
- **看不到回测数据**：确认 enabled 和 allow_backtest 均为 true，网站新版已发布；关闭演示开关，在顶部“数据来源”选择对应回测。更新前的历史回测不会自动补传，需要重新运行。
