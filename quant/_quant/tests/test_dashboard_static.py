...
class StaticBuildTests(unittest.TestCase):
    ...
    def test_backtests_are_labelled_and_separate_from_simulation_and_each_other(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.write_batch(root / 'inbox', batch())
            for run_id, bid, value in [('1' * 32, 'e' * 32, 200000), ('2' * 32, 'f' * 32, 300000)]:
                data = batch(bid)
                data.update(mode='backtest', run_id=run_id, runtime_mode='full_backtest')
                data['events'][-1]['text'] = '2026-09-08 15:01:00 - INFO - 【多策略收盘日志】总资产：' + str(value)
                self.write_batch(root / 'inbox/backtests' / run_id, data)
            build(MODULES / 'site', root / 'inbox', root / 'quant')
            index = json.loads((root / 'quant/data/index.json').read_text())
            self.assertEqual(len(index['backtests']), 2)
            self.assertEqual(json.loads((root / 'quant/data/days/2026-09-08.json').read_text())['account']['total_value'], 100100)
            for run_id, value in [('1' * 32, 200000), ('2' * 32, 300000)]:
                day = json.loads((root / 'quant/data/backtests' / run_id / 'days/2026-09-08.json').read_text())
                self.assertEqual(day['run_mode'], 'backtest')
                self.assertEqual(day['run_id'], run_id)
                self.assertEqual(day['account']['total_value'], value)
                self.assertEqual(len(day['trades']), 1)
