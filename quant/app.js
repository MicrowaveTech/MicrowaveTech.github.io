...
async function api(path,options) {
  if(STATIC_SITE){
    ...
    if(route.pathname.endsWith('/index')||route.pathname==='/api/refresh')path=`./data/${demo?'demo/':''}index.json`;
    else if(route.pathname.endsWith('/day')){const date=route.searchParams.get('date');if(!/^\d{4}-\d{2}-\d{2}$/.test(date||''))throw new Error('日期无效');path=`./data/${demo?'demo/':''}days/${date}.json`;}
...
function endpoint(part) { return `/api/${state.demo?'demo/':''}${part}`; }
...
async function loadIndex({refresh=false,provided=null}={}) {
  ...
  try {
    const index=provided||await api(refresh&&!mode?'/api/refresh':endpoint('index'),refresh&&!mode?{method:'POST'}:undefined);
